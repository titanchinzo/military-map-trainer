...
import { APPROXIMATION_NOTE, BRANCH_CATEGORY, SYMBOLS } from "@/lib/symbols";
...
const BRANCH_OPTIONS = SYMBOLS.filter(
  (s) => s.category === BRANCH_CATEGORY && s.glyph,
);
      ...
      {readOnly ? (
        placement.designation ? (
          <p className="text-[11px] text-zinc-600">
            <span className="font-medium">Нэр / дугаар:</span>{" "}
            {placement.designation}
          </p>
        ) : null
      ) : (
